...
def load_previous_module():
    ...
    for number in range(7, -1, -1):
        file_path = os.path.join(
            current_dir,
            f"{number}-build_decision_tree.py",
        )
...
_previous_module = load_previous_module()

Node = _previous_module.Node
Leaf = _previous_module.Leaf


class Decision_Tree(_previous_module.Decision_Tree):
    """Decision tree using the Gini split criterion."""
